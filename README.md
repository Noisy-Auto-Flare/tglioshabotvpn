...
3. После оплаты пользователь нажимает "Проверить", и система проверяет транзакцию через TonCenter API.
4. Опционально укажите `TONCENTER_API_KEY` для повышения лимитов запросов.
...
### Очистка базы данных
Если вам нужно полностью очистить базу данных (удалить всех пользователей, платежи и подписки) и сбросить ее к начальному состоянию:
```bash
python reset_db.py
```
Для автоматического подтверждения (без запроса y/n):
```bash
